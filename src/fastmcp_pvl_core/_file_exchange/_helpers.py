...
def register_file_exchange(
    ...
    *,
    ...
    # setdefault chain (not blind ``=``) so a future FastMCP version that
    # pre-populates other sub-keys under ``experimental_capabilities["tasks"]``
    # is not silently clobbered by this registration.
    _tasks = mcp.experimental_capabilities.setdefault("tasks", {})
    _tasks_requests = _tasks.setdefault("requests", {})
    _tasks_requests.setdefault("tools", {})["call"] = True
    ...
    def _wrap(
        ...
        async def _wrapped(*args: Any, **kwargs: Any) -> TransferHandle:
            ...
            if not isinstance(result, tuple) or len(result) != 2:
                raise TypeError(
                    f"register_file_exchange_provider({tool_name!r}): the "
                    f"decorated function must return a 2-tuple "
                    f"(ArtifactMetadata, key: str); got {type(result).__name__}"
                )
            metadata, key = result
...
def register_file_exchange_fetcher(
    ...
    async def _consume_transfer(handle: TransferHandle) -> None:
        """Consume a TransferHandle into this server's sink.

        Selects a source descriptor from the handle and dispatches to
        the per-transport fetcher.
        """
        # ``is_accessible=None`` tells ``select_source`` to skip every
        # filesystem descriptor (per its docstring); the filesystem
        # branch below is therefore only reachable when a volume_map
        # was passed to ``register_file_exchange``.
        is_accessible = (
        ...
        if isinstance(descriptor, FilesystemSource):
            # ``select_source`` only returns a FilesystemSource when
            # ``is_accessible`` is a real callable, i.e. when
            # ``fxctx.volume_map is not None``. The assertion narrows
            # the type for mypy; it cannot fire at runtime.
            assert fxctx.volume_map is not None
...
def register_file_exchange_receiver(
    ...
    def _wrap(
        ...
        async def _wrapped(*args: Any, **kwargs: Any) -> IntakeTicket:
            ...
            if not isinstance(result, tuple) or len(result) != 2:
                raise TypeError(
                    f"register_file_exchange_receiver({tool_name!r}): the "
                    f"decorated function must return a 2-tuple "
                    f"(artifact_id: str, ArtifactConstraints | None); "
                    f"got {type(result).__name__}"
                )
            artifact_id, expected = result
...
def register_file_exchange_sender(
    ...
    async def _send_to_receiver(ticket: IntakeTicket, key: str) -> None:
        """Send a local artifact to a peer via an IntakeTicket.

        ``key`` identifies the local artifact in this server's source.
        Selects a sink descriptor from the ticket and dispatches to
        the per-transport sender.
        """
        # ``is_accessible=None`` tells ``select_sink`` to skip every
        # filesystem descriptor (per its docstring); the filesystem
        # branch below is therefore only reachable when a volume_map
        # was passed to ``register_file_exchange``.
        is_accessible = (
        ...
        if isinstance(descriptor, FilesystemSink):
            # ``select_sink`` only returns a FilesystemSink when
            # ``is_accessible`` is a real callable, i.e. when
            # ``fxctx.volume_map is not None``. The assertion narrows
            # the type for mypy; it cannot fire at runtime.
            assert fxctx.volume_map is not None
