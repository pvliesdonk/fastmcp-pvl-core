...
description: How HTTP, gRPC, OpenTelemetry, GraphQL, JSON-RPC and six language error models separate a valid negative outcome (not found, permission denied, conflict, empty) from a fault, by fault attribution, wire channel, telemetry and log level.
...
- Depended on by: the same readers as the companion page. No pvl-core
