...
subject_version: "MCP 2026-07-28 (legacy 2025-11-25); FastMCP 4.0.5 and 4.0.0; mcp 2.2.0 and 2.1.1; client docs as of 2026-09-23"
...
- Covers: the `description`, `title` and annotation fields of tools,
  ...
  FastMCP 4's docstring mapping; skills served over MCP (SEP-2640) as a
  place for long-form workflow prose.
- Does not cover: tool *result* design beyond the vendor guidance quoted;
  elicitation; MCP Apps UI resources; prompt caching; pvl-core's own
  instructions builder (`_instructions.py`), which is this repository's
  behaviour, not the world's.
- Depended on by: the `writing-model-facing-text` skill, and every module
  that registers model-facing text on a downstream's server:
  `_server_info.py` (`get_server_info`), `_jobs/register.py`
  (`get_job_result` and its workflow snippet), `_transfer/register.py`
  (the link tools and their workflow snippet) and `_instructions.py` (the
  2,048-unit client boundary). The same page ships to downstream servers
  through `fastmcp-server-template`, whose scaffold `tools.py`,
  `resources.py` and `prompts.py` depend on it too.
...
- Claude Code "truncates each tool description and each server's
  ...
  JavaScript string units, so the limit is measured in UTF-16 code units.
  [source: claude-code-mcp] [source: claude-code-env]
  [pins: tests/test_instructions.py::TestBudgets::test_utf16_code_units_handles_bmp_and_astral_text,
  tests/test_instructions.py::TestBudgets::test_final_client_limit_does_not_warn_at_exact_boundary,
  tests/test_instructions.py::TestBudgets::test_final_client_limit_warns_one_unit_over_with_role_breakdown]
  No test yet holds pvl-core's registered tool descriptions to the cut.
...
### How FastMCP builds the fields (4.0.5, re-observed on 4.0.0)

Every `[observed: ...]` claim in this section and the skills-provider
listing below was made on fastmcp 4.0.5 and re-run on fastmcp 4.0.0 with
mcp 2.1.1, the versions this repository's `uv.lock` pins, with the same
wire text; the probes are the listings each marker describes, run through
`fastmcp.Client` on CPython 3.10.
...
- A test holding the tools pvl-core registers (`get_server_info`,
  `get_job_result`, the transfer link tools) to the FastMCP claims above.
  The claims are observed, not pinned, in this repository; the pins come
  with the change that applies the `writing-model-facing-text` skill to
  pvl-core's own descriptions.
