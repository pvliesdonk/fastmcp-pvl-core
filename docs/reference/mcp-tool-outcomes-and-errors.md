...
subject_version: "MCP 2026-07-28 (with 2025-11-25 and 2025-06-18); FastMCP 4.0.9 and 4.0.0; mcp 2.2.0 and 2.1.1; fastmcp-pvl-core 9.0.1; vendor docs and published servers as of 2026-09-25"
...
sources:
  ...
  - id: pvl-core-src
    ...
    resource: https://github.com/pvliesdonk/fastmcp-pvl-core/tree/v9.0.1/src/fastmcp_pvl_core
    accessed: 2026-09-25
...
server's logs. It records the world, not a decision. pvl-core's own rule
for which outcomes set `isError` is the `designing-tool-outcomes` skill,
which links back here. How older protocols, telemetry conventions and language error
...
- Depended on by: the request-logging middleware
  (`_logging_middleware.py`), the tools pvl-core registers on every
  downstream (`_jobs/register.py`, `_transfer/register.py`,
  `_server_info.py`), and the `designing-tool-outcomes` skill. No test
  asserts the level of any record this page describes, so no claim carries
  a pin.
...
### How FastMCP 4.0.9 maps a tool's behaviour to the wire and the logs (re-observed on 4.0.0)
...
- The four ways a tool can end, as they reach the wire and the logs
  ...
  log records captured on the `fastmcp` logger, 2026-09-25; every row
  re-observed on FastMCP 4.0.0, mcp 2.1.1 and CPython 3.14, including the
  masked `ValueError` text]:
...
### How the MCP Python SDK 2.2.0 separates the same cases (re-read on 2.1.1)
...
- **In the logs**, the frameworks do separate "anticipated" from "crash":
  ...
  - pvl-core's middleware does not separate them at all (see "Where this
    project departs from the subject");
...
- Two of the tools pvl-core registers report a caller-side refusal at
  ...
  `validate` hook's exception reach FastMCP's catch-all, which logs a
  traceback and, under `mask_error_details`, hides the reason from the
  model. [source: pvl-core-src] (`_jobs/register.py`,
  `_transfer/register.py`) [observed: in-memory server on FastMCP 4.0.0
