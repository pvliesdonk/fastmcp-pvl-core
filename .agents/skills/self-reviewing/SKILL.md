...
withholds permission to push, and nothing requires it; deterministic CI
results are what a merge is judged on. What it asks instead is honest
convergence: every finding you report gets fixed or explicitly justified,
...
1. Resolve the base ref, in this order of authority:
   ...
   - only as fallback, `main`: releases are cut from `main` and CI runs
     only on PRs targeting it. A stacked branch reviewed against `main`
     sweeps in its parent's whole diff, so name the parent explicitly
     when there is one.
...
- **Project review rules live in `AGENTS.md` and `CONTRIBUTING.md`.**
  There is no separate `REVIEW.md`; charter 1 below lists the pvl-core
  rules a diff most often breaks.
...
files: `AGENTS.md` (and any nested ones on the changed paths) and
`CONTRIBUTING.md`, plus the `logging-standard` skill when the diff
touches a logging call. Flag explicit-rule violations and contradictions
of documented conventions the diff introduces. The rules a pvl-core diff
most often breaks:

- a new kwarg on a `register_*` helper, `Build*` factory, or middleware
  constructor that fails the classification test: an override of a shape
  decision, an operator setting that belongs in an env var, or a
  docstring that does not name the kwarg's category;
- a foldability break: an absolute intra-package import, a runtime lookup
  of pvl-core's own distribution or package name, identity hard-coded
  instead of parameterized, an internal name exported without a `_`
  prefix, or an unintended `__all__` change;
- a `docs/specs/` edit that adds an implementation choice, or amends a
  published spec version in place;
- an operator-supplied URL or token reaching a log line or exception
  message unredacted, on any emit path, not only the one the diff fixed;
- a `!` the breaking-change policy does not support, or a breaking change
  without one.

Don't flag: guidance clearly aimed at writing rather than reviewable
invariants, style not covered by an explicit rule, anything a linter
would catch.
...
**5. Normative conformance** — only when the diff touches a `docs/specs/`
document, a formal schema or contract (JSON Schema, OpenAPI, a wire
format), code whose behaviour a spec pins, or prose that imposes RFC-2119
requirements on independent implementations. Check testability, keyword
discipline, completeness, and — where a formal artifact backs a claim —
construct the instance the prose predicts and **execute the check**
rather than predicting it. The spec is the authority, not pvl-core's
implementation: where the two diverge, report the divergence rather than
reading the code as the intended behaviour. For a diff that touches none of these, "no normative content in this
change" is the outcome; say it and move on.
