...
`docs/reference/mcp-model-facing-text.md`; read it before arguing
with a rule.

In pvl-core this text ships on every downstream's server: the
`get_server_info`, `get_job_result` and transfer link tools, their
parameters, and the instructions snippets the `register_*` helpers add.
A sentence that fails a gate here fails on every server in the family,
and downstream cannot fix it. The same skill ships to downstreams through
`fastmcp-server-template`; keep the two copies' rules aligned.
...
schema should also carry, but not on a parameter defaulting to `None`
while pvl-core supports Python 3.10: there the description falls off
that property, so keep its entry in `Args:`. Set `annotations={"title": ..., "read_only_hint":
...}` on every tool.
...
requires_tools=(...))`, in the `register_*` helper that registers the
tools the snippet names (in a downstream scaffold, the `DOMAIN-WIRING`
block). Two sentences at
...
## Recipe: generic text with a domain note

pvl-core owns the name, parameters and description of the tools it
registers, and downstream adds domain context through a `note` hook
(`register_job_tools(note=...)`, `register_transfer_routes(download_note=...,
upload_note=...)`) appended after the generic text, never replacing it.
So the generic description must pass both gates on a server pvl-core knows
nothing about:

- Describe the tool in the terms the model sees: its parameters and its
  result. A hook name (`validate`, `sink`), an env var, a pvl-core internal
  or the package's own name is invisible to the model and fails gate 1.
- Leave the domain facts to the note: what a `ref` names on this server,
  where an upload lands. The generic text says that such a fact exists,
  not what it is.
- A note is one sentence of domain context, with the same two gates.
  It must not restate the generic text; the reader sees both, in that
  order.
- When a helper composes `description=` from a function's docstring, the
  docstring ships verbatim: keep reST markup, `Returns:` sections and
  developer prose out of it, or keep them in `#` comments.
...
Print what the client receives, not what the source says. In pvl-core,
register the helper on a bare server the way its tests do
(`kv_store_url="memory://"`, a `base_url` for transfer) and list it:

```python
mcp = FastMCP("probe")
register_job_tools(mcp, build_jobs(config, jobs_config))
async with Client(mcp) as c:
    for t in await c.list_tools():
        ...
        for name, schema in t.input_schema.get("properties", {}).items():
            print("  ", name, schema.get("description"))
```

A downstream generated from `fastmcp-server-template` carries
`tests/test_model_facing_text.py`, which fails on a leaked docstring
section in any description, a tool with no description, a `str` prompt
argument carrying the JSON-schema sentence, and a tool description or the
instructions over 2,048 units. A server with many tools adds a budget test
like markdown-vault-mcp's `tests/test_client_surface_budget.py`.
...
- In pvl-core: does generic text name a hook, a pvl-core internal or the
  package, or say something only a downstream's `note` can know? Move it.
